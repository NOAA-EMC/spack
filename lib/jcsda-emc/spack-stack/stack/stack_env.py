        ...
        if self.upstreams:
            for upstream_path in self.upstreams:
                ...
                if not re.match(".+/install/*$", upstream_path):
                    logging.warning("WARNING: Upstream path '%s' does not appear to be an 'install/' directory!" % upstream_path)
                if not os.path.isdir(upstream_path):
                    logging.warning("WARNING: Upstream path '%s' does not appear to exist!" % upstream_path)
